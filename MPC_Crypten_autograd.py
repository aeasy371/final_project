...
def run_mpc_autograd_fine_tuning(train_dataloader, val_dataloader, test_dataloader, context_manager=None, num_epochs=3, learning_rate=0.001, batch_size=32, print_freq=5, num_samples=100):
	...
	# for CIFAR-10 and CIFAR-100 datasets, just change the output to 10/100
	''' This gets changed from 10->100 depending on dataset. '''
	crypten_model = crypten.nn.Sequential(
		crypten.nn.Linear(768, 10)		
	)
	crypten_model = crypten.load('encrypted_models/CIFAR10_92_encrypted.pth')
	...
	# if rank == 0:
	# crypten.save(crypten_model.state_dict(), '2_party_model_trained.pth', save_closure=torch.save)
	# model = crypten_model.decrypt()
	# torch.save(model.state_dict(), "2_party_trained_pytorch.pth")
	# # crypten.save(crypten_model, '2_party_model_trained.pth', )
...
def train_encrypted_model(train_loader, encrypted_model, num_epochs, learning_rate, batch_size, print_freq, val_dataloader, evaluation=None, evaluation_intervals=10):
	...
	for epoch in range(num_epochs):
		...
		for video, label in train_loader:
			# print(video.shape)
			''' Needs to be uncommented for MNIST '''
